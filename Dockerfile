...
RUN apt -q update && \
    ...
    env DEBIAN_FRONTEND=noninteractive apt -q -y install build-essential git-core
...
RUN adduser --system --group --home /var/lib/redis redis && install -d -g redis -o root -m 750 /etc/redis
